...
| `p2pSegmentValidator` | Function | undefined | Segment validation callback - validates the data after it has been downloaded via P2P.<br><br>Arguments:<br>`segment` (Segment) - The segment object.<br>`peerId` (String) - The ID of the peer that the segment was downloaded from.<br><br>Returns:<br>A promise - if resolved the segment considered to be valid, if rejected the error object will be passed to `SegmentError` event.
