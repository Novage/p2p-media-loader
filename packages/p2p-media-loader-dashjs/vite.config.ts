  ...
  return {
    ...
    build: {
      ...
      lib: {
        name: "p2pml.dashjs",
        fileName: (format) =>
          `p2p-media-loader-dashjs.${format}${minify ? ".min" : ""}.js`,
        formats: ["es"],
        entry: "src/index.ts",
      },
      // Core and the parsers stay bare imports for the page's import map to
      // resolve — to one core bundle, which carries both. Inlining the
      // parsers here would ship them twice. See specs/packaging.md.
      rolldownOptions: {
        external: ["p2p-media-loader-core", "p2p-media-loader-core/dash"],
      },
    },
  };
};
